...
const Places = () => {
  ...
  const [, setLoadingGalleries] = useState<Record<string, boolean>>({});
...
const handleOpenPdf = (place: Place) => {
  ...
  if (loading) {
    return (
      <section id="places" className="py-20 relative min-h-screen overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {places.map((place) => {
              ...
              return (
                <div 
                  key={place.id}
                  className="bg-white/95 backdrop-blur-sm rounded-2xl overflow-hidden shadow-card hover:shadow-xl transition-shadow group border border-white/30"
                >
