...
export interface ModeracionError {
  ...
  detalles?: {
    puntuacion?: number;
    problemas?: (string | undefined)[]; // ✅ CORREGIDO: Permitir string | undefined
    sugerencias?: (string | undefined)[]; // ✅ CORREGIDO: Permitir string | undefined
    campoEspecifico?: 'nombre' | 'descripcion' | 'descripcion_foto' | 'imagen' | 'ambos';
    timestamp?: string;
  };
}
...
// ✅ CORREGIDO COMPLETAMENTE: Manejar errores con mejor logging y estructura
const handleError = (err: unknown): string | ModeracionError => {
  ...
  if (error?.response?.data) {
    ...
    if (errorData.error && 
      ...
      // ✅ CORREGIDO: Manejar correctamente los detalles con tipos compatibles
      const detallesCorregidos = errorData.detalles ? {
        ...errorData.detalles,
        problemas: errorData.detalles.problemas || [],
        sugerencias: errorData.detalles.sugerencias || []
      } : undefined;
    ...
    if (errorData.motivo || errorData.message) {
      ...
      return {
        ...
        detalles: errorData.detalles ? {
          ...
          problemas: [errorData.motivo || errorData.message].filter(Boolean) as string[],
...
const mostrarToastRechazo = useCallback((resultado: ModeracionError | string) => {
  ...
  // ✅ MEJORADO: Construir descripción detallada con filtrado de undefined
  if (resultado.detalles?.problemas && resultado.detalles.problemas.length > 0) {
    const problemasFiltrados = resultado.detalles.problemas.filter((p): p is string => p !== undefined);
    if (problemasFiltrados.length > 0) {
      description += `\n\nProblemas detectados:\n• ${problemasFiltrados.join('\n• ')}`;
    }
  }
  
  if (resultado.detalles?.sugerencias && resultado.detalles.sugerencias.length > 0) {
    const sugerenciasFiltradas = resultado.detalles.sugerencias.filter((s): s is string => s !== undefined);
    if (sugerenciasFiltradas.length > 0) {
      description += `\n\nSugerencias:\n• ${sugerenciasFiltradas.join('\n• ')}`;
    }
  }
    ...
    try {
      ...
      if (typeof errorResult === 'object' && 'detalles' in errorResult) {
        return {
          ...
          // En la función validarCambiosLugar, dentro del catch
detalles: {
  problemas: (errorResult.detalles?.problemas?.filter((p): p is string => p !== undefined) || ['Contenido no aprobado']) as string[],
  sugerencias: (errorResult.detalles?.sugerencias?.filter((s): s is string => s !== undefined) || [
  ...
  try {
    ...
    if (!resultadoValidacion.esAprobado) {
      ...
      const errorModeracion: ModeracionError = {
        ...
        detalles: {
          problemas: resultadoValidacion.detalles?.problemas as (string | undefined)[],
          sugerencias: resultadoValidacion.detalles?.sugerencias as (string | undefined)[],
