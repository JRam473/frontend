// components/ExperienceMural.tsx (VERSIÓN CORREGIDA)
...
const ExperienceSkeletonGrid = ({ count }: { count: number }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    {Array.from({ length: count }).map((_, index) => (
      <div key={index} className="bg-white/80 backdrop-blur-sm rounded-2xl overflow-hidden shadow-card border border-white/20">
...
const EmptyState = ({ 
  ...
  activeTab: 'comunidad' | 'mis-experiencias';
  onUploadClick: () => void;
  isCommunityEmpty?: boolean;
}) => {
  if (activeTab === 'comunidad' && isCommunityEmpty) {
    return (
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="text-center py-16 bg-white/80 backdrop-blur-sm rounded-3xl border border-white/30 shadow-2xl"
      >
        <div className="max-w-md mx-auto">
          ...
          <p className="text-gray-600 mb-6 text-lg">
            Aún no hay experiencias compartidas en la comunidad. 
            Sé el primero en inspirar a otros con tu aventura.
          </p>
          <div className="space-y-4">
            <Button
              onClick={onUploadClick}
              className="bg-gradient-to-r from-blue-600 to-green-600 hover:from-blue-700 hover:to-green-700 text-white font-semibold py-3 px-8 rounded-xl shadow-lg"
            >
              <Camera className="w-5 h-5 mr-2" />
              Compartir Mi Primera Experiencia
            </Button>
            <div className="text-sm text-gray-500">
              📸 Comparte fotos • 💬 Cuenta tu historia • 🌟 Inspira a la comunidad
            </div>
          </div>
        </div>
      </motion.div>
  ...
  return (
    ...
    >
      <div className="max-w-md mx-auto">
        ...
        <p className="text-gray-600 mb-6 text-lg">
          {activeTab === 'mis-experiencias'
            ? 'Comparte tu primera experiencia y forma parte de nuestra comunidad viajera.'
            : 'Descubre las historias y momentos compartidos por otros aventureros.'
          }
        </p>
        <Button
          onClick={onUploadClick}
...
const UserStatsBanner = ({ myExperiences }: { myExperiences: Experience[] }) => {
  ...
  return (
    <motion.div 
      ...
      className="bg-gradient-to-r from-blue-50/80 to-green-50/80 backdrop-blur-sm rounded-xl p-4 mb-6 border border-white/30"
    >
...
// Interface para el resultado de análisis de imagen
...
const ExperienceEditModal = ({ 
  ...
  errorModelo: string | null;
  analizarImagen: (file: File) => Promise<AnalisisResultado>;
}) => {
...
export const ExperienceMural = () => {
  ...
  // 🆕 Estados para el banner de invitación
  ...
  // 🆕 Efecto para el banner de invitación
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    initializeData();
  }, [fetchExperiences, fetchMyExperiences, inicializarModelo, startAutoRefresh]);

  // 🆕 Cambio de pestaña optimizado
  const handleTabChange = useCallback((tab: 'comunidad' | 'mis-experiencias') => {
    setActiveTab(tab);
  }, []);
  ...
  const isCommunityEmpty = activeTab === 'comunidad' && experiences.length === 0 && !loading;
  ...
  const handleUpload = async () => {
    ...
    if (result.success) {
      ...
      toast({
        ...
        description: 'Tu experiencia ha sido publicada exitosamente.',
        variant: 'default',
      });
      ...
      setPendingExperience({
        ...
        nombreUsuario: uploadData.nombreUsuario
      });
      setShowTermsDialog(true);
    }
  };

  // Mantener las demás funciones handleEdit, handleDelete, etc. iguales
  ...
  const handleTermsAccept = async () => {
    if (pendingExperience) {
      ...
      const result = await uploadExperienceWithValidation(
        ...
        pendingExperience.nombreUsuario
      );
    ...
    return (
      <section className="py-20 relative min-h-screen overflow-hidden">
        ...
        <div 
          ...
          style={{ 
            backgroundImage: 'url(https://res.cloudinary.com/dinsl266g/image/upload/v1763074398/fondo-galeria_urye7x.svg)',
            backgroundSize: 'cover',
            backgroundPosition: 'center',
            backgroundRepeat: 'no-repeat',
            filter: 'blur(6px) brightness(0.85) saturate(0.8)'
          }}
        />
        
        {/* 🎨 OVERLAY - SEGUNDO ELEMENTO */}
        <div className="absolute inset-0 bg-black/45 backdrop-blur-[1px] z-0" />
        
        {/* 🎨 CONTENIDO - TERCER ELEMENTO CON z-10 */}
  ...
  return (
    <>
      <section className="py-20 relative min-h-screen overflow-hidden">
        {/* 🆕 FONDO SVG CON DIFUMINADO OPTIMIZADO */}
        <div 
          className="absolute inset-0 bg-cover bg-center z-0"
          style={{ 
            ...
            backgroundPosition: 'center',
            backgroundRepeat: 'no-repeat',
            filter: 'blur(4px) brightness(0.9)' // 🆕 REDUCIDO EL DIFUMINADO
          }}
        />
        
        {/* 🎨 OVERLAY MEJORADO - MÁS OSCURO PARA MEJOR LEGIBILIDAD */}
        <div className="absolute inset-0 bg-black/60 backdrop-blur-[1px] z-0" />
        
        {/* 🎨 CONTENIDO - TERCER ELEMENTO CON z-10 */}
        <div className="container mx-auto px-4 relative z-10">
          {/* Encabezado CON COLORES MEJORADOS */}
          <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            <div className="text-center md:text-left">
              <h2 className="text-4xl font-bold text-white mb-2"> {/* 🆕 TEXTO BLANCO */}
                Mural de <span className="text-blue-300">Experiencias</span> {/* 🆕 AZUL CLARO */}
              </h2>
              <p className="text-xl text-gray-200"> {/* 🆕 TEXTO GRIS CLARO */}
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                ...
                className="bg-gradient-to-r from-blue-500 to-green-500 hover:from-blue-600 hover:to-green-600 text-white font-semibold flex items-center gap-2 px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300"
              >
          ...
          {/* 🆕 AUTO REFRESH INDICATOR CON COLORES MEJORADOS */}
          <div className="flex items-center gap-2 text-sm text-gray-300 mb-4"> {/* 🆕 TEXTO GRIS CLARO */}
            <div className={`w-2 h-2 rounded-full ${autoRefresh ? 'bg-green-400 animate-pulse' : 'bg-gray-500'}`} />
            <span>Actualización automática: {autoRefresh ? 'Activada' : 'Desactivada'}</span>
            <Button
              variant="ghost"
              size="sm"
              onClick={() => handleAutoRefreshToggle(!autoRefresh)}
              className="h-6 px-2 text-xs text-gray-300 hover:text-white hover:bg-white/10" /* 🆕 COLORES MEJORADOS */
            >
          ...
          {/* 🆕 PESTAÑAS MEJORADAS */}
          <div className="flex border-b border-gray-600 mb-6 bg-white/10 backdrop-blur-sm rounded-t-xl p-2"> {/* 🆕 FONDO SEMITRANSPARENTE */}
            <button
              onClick={() => handleTabChange('comunidad')}
              className={`px-6 py-3 font-medium text-lg rounded-lg transition-all ${
                activeTab === 'comunidad'
                  ? 'text-white bg-blue-500/30 shadow-sm' /* 🆕 COLORES MEJORADOS */
                  : 'text-gray-300 hover:text-white hover:bg-white/10' /* 🆕 COLORES MEJORADOS */
              }`}
            >
              👥 Comunidad
            </button>
            <button
              onClick={() => handleTabChange('mis-experiencias')}
              className={`px-6 py-3 font-medium text-lg rounded-lg transition-all ${
                activeTab === 'mis-experiencias'
                  ? 'text-white bg-blue-500/30 shadow-sm' /* 🆕 COLORES MEJORADOS */
                  : 'text-gray-300 hover:text-white hover:bg-white/10' /* 🆕 COLORES MEJORADOS */
          ...
          {activeTab === 'comunidad' && (
            <motion.div 
              ...
              className="relative bg-gradient-to-r from-blue-500/30 via-green-500/30 to-emerald-500/30 rounded-2xl p-6 mb-8 overflow-hidden border border-white/20 backdrop-blur-sm" /* 🆕 COLORES MEJORADOS */
            >
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex-1 text-center md:text-left">
                  <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
                    <Camera className="w-6 h-6 text-blue-300" /> {/* 🆕 COLOR MEJORADO */}
                    <h3 className="text-xl font-bold text-white">Mural de Experiencias</h3> {/* 🆕 TEXTO BLANCO */}
                  </div>
                  ...
                  <AnimatePresence mode="wait">
                    <motion.p
                      ...
                      className="text-lg text-gray-200 mb-4" /* 🆕 TEXTO GRIS CLARO */
                    >
                  ...
                  <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                    <span className="px-3 py-1 bg-blue-500/30 text-blue-200 rounded-full text-sm">#Aventuras</span> {/* 🆕 COLORES MEJORADOS */}
                    <span className="px-3 py-1 bg-green-500/30 text-green-200 rounded-full text-sm">#Naturaleza</span> {/* 🆕 COLORES MEJORADOS */}
                    <span className="px-3 py-1 bg-emerald-500/30 text-emerald-200 rounded-full text-sm">#Comunidad</span> {/* 🆕 COLORES MEJORADOS */}
                  </div>
                </div>
                ...
                <motion.div
                  ...
                  whileTap={{ scale: 0.95 }}
                >
                  <Users className="w-16 h-16 text-blue-400/40" /> {/* 🆕 COLOR MEJORADO */}
                </motion.div>
              </div>
            </motion.div>
          )}
                  ...
                  <AnimatePresence>
                    {displayedExperiences.map((experience, index) => (
                      <motion.div
                        ...
                        className="bg-white/95 backdrop-blur-sm rounded-2xl overflow-hidden shadow-card hover:shadow-2xl transition-all duration-300 group relative border border-white/30"
                      >
