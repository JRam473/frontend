...
export const MapRouteViewer: React.FC<MapRouteViewerProps> = ({
  ...
  // ✅ CORREGIDO: useRef con valor inicial null
  ...
  const initializeDefaultRoute = useCallback(async () => {
    try {
      ...
      // ✅ CORREGIDO: Asegurar que optimalRoute sea del tipo RouteType
      setRouteType(optimalRoute as RouteType);
  ...
  // ✅ CORREGIDO: Limpieza segura del mapa
  const cleanupMap = useCallback(() => {
    try {
      ...
      if (directionsRendererRef.current) {
        ...
        // ✅ CORREGIDO: Usar setDirections correctamente
  ...
  // ✅ CORREGIDO: Función para agregar marcadores
  ...
  // ✅ CORREGIDO: Renderizar ruta en el mapa
  const renderRouteOnMap = useCallback((segments: RouteSegment[]) => {
    ...
    try {
      // ✅ CORREGIDO: Limpiar ruta anterior correctamente
      directionsRendererRef.current.setDirections({ routes: [] } as unknown as google.maps.DirectionsResult);
  ...
  // ✅ Efecto principal para cargar/limpiar mapa
  useEffect(() => {
    if (isOpen) {
      loadMap();
    } else {
  ...
  // ✅ Renderizar ruta cuando hay segmentos
  useEffect(() => {
  ...
  // ✅ Actualizar visibilidad de ruta
  useEffect(() => {
